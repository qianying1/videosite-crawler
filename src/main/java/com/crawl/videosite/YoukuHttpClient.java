...
import com.crawl.proxy.YoukuProxyHttpClient;
 ...
 */
public class YoukuHttpClient extends AbstractHttpClient implements IHttpClient {
    private static Logger logger = LoggerFactory.getLogger(Main.class);
    private volatile static YoukuHttpClient instance;
    /**
     * 统计用户数量
     */
    ...
    public static YoukuHttpClient getInstance() {
        if (instance == null) {
            synchronized (YoukuHttpClient.class) {
                if (instance == null) {
                    instance = new YoukuHttpClient();
                }
            }
        }
        return instance;
    ...
    private void intiThreadPool() {
        ...
        detailPageThreadPool = new SimpleThreadPoolExecutor(Config.downloadThreadSize,
                ...
                "youkuDetailPageThreadPool");

        //列表页线程池
        listPageThreadPool = new SimpleThreadPoolExecutor(50, 80,
                0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(5000),
                new ThreadPoolExecutor.DiscardPolicy(), "youkuListPageThreadPool");
        new Thread(new ThreadPoolMonitor(detailPageThreadPool, "YoukuDetailPageDownloadThreadPool")).start();
        new Thread(new ThreadPoolMonitor(listPageThreadPool, "YoukuListPageDownloadThreadPool")).start();
        detailListPageThreadPool = new SimpleThreadPoolExecutor(Config.downloadThreadSize,
                Config.downloadThreadSize,
                0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(2000),
                new ThreadPoolExecutor.DiscardPolicy(),
                "youkuDetailListPageThreadPool");
        new Thread(new ThreadPoolMonitor(detailListPageThreadPool, "YoukuDetailListPageThreadPool")).start();
    ...
    public void startCrawl(String url) {
        detailPageThreadPool.execute(new BiliBiliDetailPageTask(url, Config.youkuIsProxy));
        manageHttpClient();
    }
    ...
    public void startCrawl() {
        ...
        request.setHeader("authorization", "oauth " + YoukuHttpClient.getAuthorization());
        detailListPageThreadPool.execute(new BiliBiliDetailListPageTask(request, Config.youkuIsProxy));
        manageHttpClient();
    }
    ...
    private String initAuthorization() {
        ...
        BiliBiliGeneralPageTask generalPageTask = new BiliBiliGeneralPageTask(Config.youkuStartURL, true);
        generalPageTask.run();
    ...
    public void manageHttpClient() {
        while (true) {
            ...
            if (detailListPageThreadPool.isTerminated()) {
                ...
                //关闭代理检测线程池
                YoukuProxyHttpClient.getInstance().getProxyTestThreadExecutor().shutdownNow();
                //关闭代理下载页线程池
                YoukuProxyHttpClient.getInstance().getProxyDownloadThreadExecutor().shutdownNow();

                break;
            }
            double costTime = (System.currentTimeMillis() - startTime) / 1000.0;//单位s
            logger.debug("优酷抓取速率：" + parseUserCount.get() / costTime + "个/s");
