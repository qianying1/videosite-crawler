 ...
 */
public class TudouDetailPageTask extends TudouAbstractPageTask {
    private static Logger logger = LoggerFactory.getLogger(TudouDetailPageTask.class);
    private static DetailPageParser proxyDetailPageParser;
    static {
        proxyDetailPageParser = getProxyDetailParser();
    }

    public TudouDetailPageTask(String url, boolean proxyFlag) {
        super(url, proxyFlag);
    }

    @Override
    void retry() {
        commonHttpClient.getDetailPageThreadPool().execute(new TudouDetailPageTask(url, Config.isProxy));
    }
...
//            videoSiteDao1.insertUser(u);
    ...
    private void handleUrl(String url){
        ...
        if(!Config.dbEnable){
            commonHttpClient.getListPageThreadPool().execute(new TudouListPageTask(request, Config.isProxy));
            return ;
        }
        commonHttpClient.getListPageThreadPool().execute(new TudouListPageTask(request, Config.isProxy));
    }
