 ...
 * 知乎用户列表详情页task
 */
public class TudouDetailListPageTask extends TudouAbstractPageTask {
    private static Logger logger = LoggerFactory.getLogger(TudouDetailListPageTask.class);
    ...
    public TudouDetailListPageTask(HttpRequestBase request, boolean proxyFlag) {
    ...
    @Override
    void retry() {
        commonHttpClient.getDetailListPageThreadPool().execute(new TudouDetailListPageTask(request, Config.isProxy));
    }
            ...
            if(Config.dbEnable){
                ...
                for (int j = 0; j < u.getFollowees() / 20; j++){
                    ...
                    if (videoSiteDao1.insertUrl(cn, Md5Util.Convert2Md5(nextUrl)) ||
                        ...
                        commonHttpClient.getDetailListPageThreadPool().execute(new TudouDetailListPageTask(request, true));
                    }
                }*/
            }
