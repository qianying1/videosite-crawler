...
public abstract class AbstractPageTaskCommon implements Runnable {
    ...
    public static void setProxyTimesLog(Proxy proxy, String logStr, Logger logger, Long requestEndTime, Long requestStartTime) {
        logger.debug(logStr);
        proxy.setSuccessfulTimes(proxy.getSuccessfulTimes() + 1);
        proxy.setSuccessfulTotalTime(proxy.getSuccessfulTotalTime() + (requestEndTime - requestStartTime));
        double aTime = (proxy.getSuccessfulTotalTime() + 0.0) / proxy.getSuccessfulTimes();
        proxy.setSuccessfulAverageTime(aTime);
