 ...
 */
public class PptvDetailPageTask extends PptvAbstractPageTask {
    private static Logger logger = LoggerFactory.getLogger(PptvDetailPageTask.class);
    ...
    public PptvDetailPageTask(String url, boolean proxyFlag) {
        super(url, proxyFlag);
    }
...
//            VideoSiteDAO.insertUser(u);
    ...
    private void handleUrl(String url) {
        ...
        if (!Config.dbEnable) {
            commonHttpClient.getListPageThreadPool().execute(new PptvListPageTask(request, Config.isProxy));
            return;
        }
        commonHttpClient.getListPageThreadPool().execute(new PptvListPageTask(request, Config.isProxy));
    }
