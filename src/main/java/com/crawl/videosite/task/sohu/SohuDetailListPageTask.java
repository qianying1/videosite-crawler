 ...
 * 知乎用户列表详情页task
 */
public class SohuDetailListPageTask extends SohuAbstractPageTask {
    private static Logger logger = LoggerFactory.getLogger(SohuDetailListPageTask.class);
    ...
    public SohuDetailListPageTask(HttpRequestBase request, boolean proxyFlag) {
    ...
    @Override
    void retry() {
        commonHttpClient.getDetailListPageThreadPool().execute(new SohuDetailListPageTask(request, Config.isProxy));
    }
            ...
            if(Config.dbEnable){
                ...
                for (int j = 0; j < u.getFollowees() / 20; j++){
                    ...
                    if (videoSiteDao1.insertUrl(cn, Md5Util.Convert2Md5(nextUrl)) ||
                        ...
                        commonHttpClient.getDetailListPageThreadPool().execute(new SohuDetailListPageTask(request, true));
                    }
                }
            }
