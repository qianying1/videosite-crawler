 ...
 * @see ProxyPool
 */
public abstract class LetvAbstractPageTask implements Runnable{
	private static Logger logger = LoggerFactory.getLogger(LetvAbstractPageTask.class);
	...
	public LetvAbstractPageTask(){

	}
	public LetvAbstractPageTask(String url, boolean proxyFlag){
		this.url = url;
		this.proxyFlag = proxyFlag;
	}
	public LetvAbstractPageTask(HttpRequestBase request, boolean proxyFlag){
		...
		try {
			...
			if(url != null){
				if (proxyFlag){
					...
					currentProxy = ProxyPool.letvProxyQueue.take();
			...
			if (currentProxy != null && !ProxyUtil.isDiscardProxy(currentProxy)){
				...
				ProxyPool.letvProxyQueue.add(currentProxy);
			}
		}
	}
