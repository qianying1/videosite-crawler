 ...
 */
public class LetvDetailPageTask extends LetvAbstractPageTask {
    private static Logger logger = LoggerFactory.getLogger(LetvDetailPageTask.class);
    ...
    public LetvDetailPageTask(String url, boolean proxyFlag) {
        super(url, proxyFlag);
    }

    @Override
    void retry() {
        commonHttpClient.getDetailPageThreadPool().execute(new LetvDetailPageTask(url, Config.isProxy));
    }
...
//            videoSiteDao1.insertUser(u);
    ...
    private void handleUrl(String url) {
        ...
        if (!Config.dbEnable) {
            commonHttpClient.getListPageThreadPool().execute(new LetvListPageTask(request, Config.isProxy));
            return;
        }
        commonHttpClient.getListPageThreadPool().execute(new LetvListPageTask(request, Config.isProxy));
    }
