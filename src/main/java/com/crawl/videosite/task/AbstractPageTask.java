...
public abstract class AbstractPageTask implements Runnable{
	...
	protected static VideoSiteHttpClient videoSiteHttpClient = VideoSiteHttpClient.getInstance();
	static {
		...
		try {
			...
			if(url != null){
				if (proxyFlag){
					...
					page = videoSiteHttpClient.getWebPage(tempRequest);
				}else {
					requestStartTime = System.currentTimeMillis();
					page = videoSiteHttpClient.getWebPage(url);
				}
				...
				if (proxyFlag){
					...
					requestStartTime = System.currentTimeMillis();
					page = videoSiteHttpClient.getWebPage(request);
				}else {
					requestStartTime = System.currentTimeMillis();
					page = videoSiteHttpClient.getWebPage(request);
				}
            ...
            if(!videoSiteHttpClient.getDetailListPageThreadPool().isShutdown()){
				retry();
			}
		} finally {
			if (request != null){
