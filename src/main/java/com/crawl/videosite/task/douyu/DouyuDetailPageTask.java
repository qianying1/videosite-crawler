 ...
 */
public class DouyuDetailPageTask extends DouyuAbstractPageTask {
    private static Logger logger = LoggerFactory.getLogger(DouyuDetailPageTask.class);
    ...
    public DouyuDetailPageTask(String url, boolean proxyFlag) {
        super(url, proxyFlag);
    }
...
//            VideoSiteDAO.insertUser(u);
    ...
    private void handleUrl(String url) {
        ...
        if (!Config.dbEnable) {
            commonHttpClient.getListPageThreadPool().execute(new DouyuListPageTask(request, Config.isProxy));
            return;
        }
        commonHttpClient.getListPageThreadPool().execute(new DouyuListPageTask(request, Config.isProxy));
    }
