 ...
 */
public class DetailPageTaskCommon extends AbstractPageTaskCommon {
    private static Logger logger = LoggerFactory.getLogger(DetailPageTaskCommon.class);
    private static DetailPageParser proxyDetailPageParser;
    static {
        proxyDetailPageParser = getProxyDetailParser();
    }

    public DetailPageTaskCommon(String url, boolean proxyFlag) {
        super(url, proxyFlag);
    }
...
//            VideoSiteDAO.insertUser(u);
    ...
    private void handleUrl(String url){
        ...
        if(!Config.dbEnable){
            commonHttpClient.getListPageThreadPool().execute(new ListPageTaskCommon(request, Config.isProxy));
            return ;
        }
        commonHttpClient.getListPageThreadPool().execute(new ListPageTaskCommon(request, Config.isProxy));
    }
