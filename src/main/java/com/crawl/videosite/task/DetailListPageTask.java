...
import static com.crawl.videosite.VideoSiteHttpClient.parseUserCount;
...
public class DetailListPageTask extends AbstractPageTask{
    ...
    @Override
    void retry() {
        videoSiteHttpClient.getDetailListPageThreadPool().execute(new DetailListPageTask(request, Config.isProxy));
    }
        ...
        for(User u : list){
            ...
            if(Config.dbEnable){
                ...
                for (int j = 0; j < u.getFollowees() / 20; j++){
                    if (videoSiteHttpClient.getDetailListPageThreadPool().getQueue().size() > 1000){
                        continue;
                    }
                    ...
                    if (videoSiteDao1.insertUrl(cn, Md5Util.Convert2Md5(nextUrl)) ||
                            videoSiteHttpClient.getDetailListPageThreadPool().getActiveCount() == 1){
                        //防止死锁
                        HttpGet request = new HttpGet(nextUrl);
                        request.setHeader("authorization", "oauth " + VideoSiteHttpClient.getAuthorization());
                        videoSiteHttpClient.getDetailListPageThreadPool().execute(new DetailListPageTask(request, true));
                    }
                }
            }
            else if(!Config.dbEnable || videoSiteHttpClient.getDetailListPageThreadPool().getActiveCount() == 1){
                parseUserCount.incrementAndGet();
                for (int j = 0; j < u.getFollowees() / 20; j++){
                    String nextUrl = String.format(USER_FOLLOWEES_URL, u.getUserToken(), j * 20);
                    HttpGet request = new HttpGet(nextUrl);
