...
import com.crawl.proxy.LetvProxyHttpClient;
 ...
 */
public class LetvHttpClient extends AbstractHtmlUnit implements IHtmlUnit {
    ...
    private void intiThreadPool() {
        ...
        detailPageThreadPool = new SimpleThreadPoolExecutor(Config.downloadThreadSize,
                ...
                "letvDetailPageThreadPool");

        //列表页线程池
        listPageThreadPool = new SimpleThreadPoolExecutor(50, 80,
                0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(5000),
                new ThreadPoolExecutor.DiscardPolicy(), "letvListPageThreadPool");
        new Thread(new ThreadPoolMonitor(detailPageThreadPool, "LetvDetailPageDownloadThreadPool")).start();
        new Thread(new ThreadPoolMonitor(listPageThreadPool, "LetvListPageDownloadThreadPool")).start();
        detailListPageThreadPool = new SimpleThreadPoolExecutor(Config.downloadThreadSize,
                Config.downloadThreadSize,
                0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(2000),
                new ThreadPoolExecutor.DiscardPolicy(),
                "letvDetailListPageThreadPool");
        new Thread(new ThreadPoolMonitor(detailListPageThreadPool, "LetvDetailListPageThreadPool")).start();
...
//        detailListPageThreadPool.execute(new BiliBiliDetailListPageTask(request, Config.isProxy));
    ...
    private String initAuthorization() {
        ...
        BiliBiliGeneralPageTask generalPageTask = new BiliBiliGeneralPageTask(Config.letvStartURL, true);
        generalPageTask.run();
    ...
    public void manageHttpClient() {
        while (true) {
            ...
            if (detailListPageThreadPool.isTerminated()) {
                ...
                //关闭代理检测线程池
                LetvProxyHttpClient.getInstance().getProxyTestThreadExecutor().shutdownNow();
                //关闭代理下载页线程池
                LetvProxyHttpClient.getInstance().getProxyDownloadThreadExecutor().shutdownNow();

                break;
            }
            double costTime = (System.currentTimeMillis() - startTime) / 1000.0;//单位s
            logger.debug("乐视抓取速率：" + parseUserCount.get() / costTime + "个/s");
