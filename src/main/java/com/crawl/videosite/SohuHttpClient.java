...
import com.crawl.proxy.SohuProxyHttpClient;
 ...
 */
public class SohuHttpClient extends AbstractHttpClient implements IHttpClient {
    private static Logger logger = LoggerFactory.getLogger(Main.class);
    private volatile static SohuHttpClient instance;
    /**
     * 统计用户数量
     */
    ...
    public static SohuHttpClient getInstance() {
        if (instance == null) {
            synchronized (SohuHttpClient.class) {
                if (instance == null) {
                    instance = new SohuHttpClient();
                }
            }
        }
        return instance;
    ...
    private void intiThreadPool() {
        ...
        detailPageThreadPool = new SimpleThreadPoolExecutor(Config.downloadThreadSize,
                ...
                "sohuDetailPageThreadPool");

        //列表页线程池
        listPageThreadPool = new SimpleThreadPoolExecutor(50, 80,
                0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(5000),
                new ThreadPoolExecutor.DiscardPolicy(), "sohuListPageThreadPool");
        new Thread(new ThreadPoolMonitor(detailPageThreadPool, "SohuDetailPageDownloadThreadPool")).start();
        new Thread(new ThreadPoolMonitor(listPageThreadPool, "SohuListPageDownloadThreadPool")).start();
        detailListPageThreadPool = new SimpleThreadPoolExecutor(Config.downloadThreadSize,
                Config.downloadThreadSize,
                0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(2000),
                new ThreadPoolExecutor.DiscardPolicy(),
                "sohuDetailListPageThreadPool");
        new Thread(new ThreadPoolMonitor(detailListPageThreadPool, "SohuDetailListPageThreadPool")).start();
    ...
    public void startCrawl() {
        ...
        request.setHeader("authorization", "oauth " + SohuHttpClient.getAuthorization());
        detailListPageThreadPool.execute(new BiliBiliDetailListPageTask(request, Config.sohuIsProxy));
        manageHttpClient();
    }
    ...
    private String initAuthorization() {
        ...
        BiliBiliGeneralPageTask generalPageTask = new BiliBiliGeneralPageTask(Config.sohuStartURL, true);
        generalPageTask.run();
    ...
    public void manageHttpClient() {
        while (true) {
            ...
            if (detailListPageThreadPool.isTerminated()) {
                ...
                //关闭代理检测线程池
                SohuProxyHttpClient.getInstance().getProxyTestThreadExecutor().shutdownNow();
                //关闭代理下载页线程池
                SohuProxyHttpClient.getInstance().getProxyDownloadThreadExecutor().shutdownNow();

                break;
            }
            double costTime = (System.currentTimeMillis() - startTime) / 1000.0;//单位s
            logger.debug("搜狐抓取速率：" + parseUserCount.get() / costTime + "个/s");
