...
public class VideoSiteUserIndexDetailPageParser implements DetailPageParser {
    private static VideoSiteUserIndexDetailPageParser videoSiteUserIndexDetailPageParser;
    public static VideoSiteUserIndexDetailPageParser getInstance(){
        if(videoSiteUserIndexDetailPageParser == null){
            videoSiteUserIndexDetailPageParser = new VideoSiteUserIndexDetailPageParser();
        }
        return videoSiteUserIndexDetailPageParser;
    }
    @Override
    ...
    private User parseUserdetail(Document doc){
        ...
        u.setUrl("https://www.videosite.com" + doc.select(".title-section a").first().attr("href"));//用户首页链接
