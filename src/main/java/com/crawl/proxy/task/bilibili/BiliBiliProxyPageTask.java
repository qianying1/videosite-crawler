...
import static com.crawl.proxy.ProxyPool.biliBiliProxyQueue;

/**
 * 下载代理网页并解析
 * 若下载失败，通过代理去下载代理网页
 */
public class BiliBiliProxyPageTask implements Runnable {
    ...
    protected static BiliBiliProxyHttpClient proxyHttpClient = BiliBiliProxyHttpClient.getInstance();

    private BiliBiliProxyPageTask() {

    }

    public BiliBiliProxyPageTask(String url, boolean proxyFlag) {
        this.url = url;
    ...
    public void run() {
        ...
        try {
            ...
            if (proxyFlag) {
                ...
                currentProxy = biliBiliProxyQueue.take();
            ...
            if (currentProxy != null) {
                ...
                biliBiliProxyQueue.add(currentProxy);
            }
            if (tempRequest != null) {
     ...
     */
    public void retry() {
        proxyHttpClient.getProxyDownloadThreadExecutor().execute(new BiliBiliProxyPageTask(url, Config.bilibiliIsProxy));
    }
        ...
        for (Proxy p : proxyList) {
            if (!CommonHttpClient.getInstance().getDetailListPageThreadPool().isTerminated()) {
                ...
                if (!containFlag) {
                    ...
                    proxyHttpClient.getProxyTestThreadExecutor().execute(new BiliBiliProxyTestTask(p));
                }
            }
        }
    }
