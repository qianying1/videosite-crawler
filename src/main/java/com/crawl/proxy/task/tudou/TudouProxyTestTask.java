 ...
 */
public class TudouProxyTestTask implements Runnable{
    private final static Logger logger = LoggerFactory.getLogger(TudouProxyTestTask.class);
    private Proxy proxy;
    public TudouProxyTestTask(Proxy proxy){
        this.proxy = proxy;
    }
    @Override
    public void run() {
        long startTime = System.currentTimeMillis();
        HttpGet request = new HttpGet(Constants.TUDOU_INDEX_URL);
        try {
            ...
            ProxyPool.tudouProxyQueue.add(proxy);
        } catch (IOException e) {
