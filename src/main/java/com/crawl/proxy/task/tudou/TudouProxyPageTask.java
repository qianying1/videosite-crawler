...
import static com.crawl.proxy.ProxyPool.tudouProxyQueue;

/**
 * 下载代理网页并解析
 * 若下载失败，通过代理去下载代理网页
 */
public class TudouProxyPageTask implements Runnable {
    ...
    protected static TudouProxyHttpClient proxyHttpClient = TudouProxyHttpClient.getInstance();

    private TudouProxyPageTask() {

    }

    public TudouProxyPageTask(String url, boolean proxyFlag) {
        this.url = url;
    ...
    public void run() {
        ...
        try {
            ...
            if (proxyFlag) {
                ...
                currentProxy = tudouProxyQueue.take();
            ...
            if (currentProxy != null) {
                ...
                tudouProxyQueue.add(currentProxy);
            }
            if (tempRequest != null) {
     ...
     */
    public void retry() {
        proxyHttpClient.getProxyDownloadThreadExecutor().execute(new TudouProxyPageTask(url, Config.tudouIsProxy));
    }
    ...
    public void handle(Page page) {
        ...
        for (Proxy p : proxyList) {
            if (!CommonHttpClient.getInstance().getDetailListPageThreadPool().isTerminated()) {
                ...
                boolean containFlag = ProxyPool.tudouProxySet.contains(p);
                ...
                if (!containFlag) {
                    ...
                    proxyHttpClient.getProxyTestThreadExecutor().execute(new TudouProxyTestTask(p));
                }
            }
        }
    }
