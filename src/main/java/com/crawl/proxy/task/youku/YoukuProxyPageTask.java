...
import static com.crawl.proxy.ProxyPool.youkuProxyQueue;

/**
 * 下载代理网页并解析
 * 若下载失败，通过代理去下载代理网页
 */
public class YoukuProxyPageTask implements Runnable {
    ...
    protected static YoukuProxyHttpClient proxyHttpClient = YoukuProxyHttpClient.getInstance();

    private YoukuProxyPageTask() {

    }

    public YoukuProxyPageTask(String url, boolean proxyFlag) {
        this.url = url;
    ...
    public void run() {
        ...
        try {
            ...
            if (proxyFlag) {
                ...
                currentProxy = youkuProxyQueue.take();
            ...
            if (currentProxy != null) {
                ...
                youkuProxyQueue.add(currentProxy);
            }
            if (tempRequest != null) {
     ...
     */
    public void retry() {
        proxyHttpClient.getProxyDownloadThreadExecutor().execute(new YoukuProxyPageTask(url, Config.youkuIsProxy));
    }
    ...
    public void handle(Page page) {
        ...
        for (Proxy p : proxyList) {
            if (!CommonHttpClient.getInstance().getDetailListPageThreadPool().isTerminated()) {
                ...
                boolean containFlag = ProxyPool.youkuProxySet.contains(p);
                ...
                if (!containFlag) {
                    ...
                    proxyHttpClient.getProxyTestThreadExecutor().execute(new YoukuProxyTestTask(p));
                }
            }
        }
    }
