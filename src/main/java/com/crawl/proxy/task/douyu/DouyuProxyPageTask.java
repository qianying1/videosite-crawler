...
import static com.crawl.proxy.ProxyPool.douyuProxyQueue;

/**
 * 下载代理网页并解析
 * 若下载失败，通过代理去下载代理网页
 */
public class DouyuProxyPageTask implements Runnable {
    private static Logger logger = LoggerFactory.getLogger(DouyuProxyPageTask.class);
    protected String url;
    private boolean proxyFlag;//是否通过代理下载
    private Proxy currentProxy;//当前线程使用的代理

    protected static DouyuProxyHttpClient proxyHttpClient = DouyuProxyHttpClient.getInstance();

    private DouyuProxyPageTask() {

    }

    public DouyuProxyPageTask(String url, boolean proxyFlag) {
        this.url = url;
    ...
    public void run() {
        ...
        try {
            ...
            if (proxyFlag) {
                ...
                currentProxy = douyuProxyQueue.take();
            ...
            if (currentProxy != null) {
                ...
                douyuProxyQueue.add(currentProxy);
            }
            if (tempRequest != null) {
     ...
     */
    public void retry() {
        proxyHttpClient.getProxyDownloadThreadExecutor().execute(new DouyuProxyPageTask(url, Config.douyuIsProxy));
    }
    ...
    public void handle(Page page) {
        ...
        for (Proxy p : proxyList) {
            if (!ProxyHttpClient.getInstance().getDetailListPageThreadPool().isTerminated()) {
                ...
                boolean containFlag = ProxyPool.douyuProxySet.contains(p);
                ...
                if (!containFlag) {
                    ...
                    proxyHttpClient.getProxyTestThreadExecutor().execute(new DouyuProxyTestTask(p));
                }
            }
        }
    }
