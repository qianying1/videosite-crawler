 ...
 */
public class SohuProxyTestTask implements Runnable {
    private final static Logger logger = LoggerFactory.getLogger(SohuProxyTestTask.class);
    private Proxy proxy;

    public SohuProxyTestTask(Proxy proxy) {
        this.proxy = proxy;
    }
    ...
    public void run() {
        ...
        HttpGet request = new HttpGet(Constants.SOHU_INDEX_URL);
        try {
            ...
            ProxyPool.sohuProxyQueue.add(proxy);
        } catch (IOException e) {
