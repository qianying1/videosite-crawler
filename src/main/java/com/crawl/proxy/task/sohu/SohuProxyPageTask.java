...
import static com.crawl.proxy.ProxyPool.sohuProxyQueue;

/**
 * 下载代理网页并解析
 * 若下载失败，通过代理去下载代理网页
 */
public class SohuProxyPageTask implements Runnable {
    ...
    protected static SohuProxyHttpClient proxyHttpClient = SohuProxyHttpClient.getInstance();

    private SohuProxyPageTask() {

    }

    public SohuProxyPageTask(String url, boolean proxyFlag) {
        this.url = url;
    ...
    public void run() {
        ...
        try {
            ...
            if (proxyFlag) {
                ...
                currentProxy = sohuProxyQueue.take();
            ...
            if (currentProxy != null) {
                ...
                sohuProxyQueue.add(currentProxy);
            }
            if (tempRequest != null) {
     ...
     */
    public void retry() {
        proxyHttpClient.getProxyDownloadThreadExecutor().execute(new SohuProxyPageTask(url, Config.sohuIsProxy));
    }
    ...
    public void handle(Page page) {
        ...
        for (Proxy p : proxyList) {
            if (!ProxyHttpClient.getInstance().getDetailListPageThreadPool().isTerminated()) {
                ...
                boolean containFlag = ProxyPool.sohuProxySet.contains(p);
                ...
                if (!containFlag) {
                    ...
                    proxyHttpClient.getProxyTestThreadExecutor().execute(new SohuProxyTestTask(p));
                }
            }
        }
    }
