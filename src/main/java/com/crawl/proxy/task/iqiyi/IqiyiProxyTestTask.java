 ...
 */
public class IqiyiProxyTestTask implements Runnable{
    private final static Logger logger = LoggerFactory.getLogger(IqiyiProxyTestTask.class);
    private Proxy proxy;
    public IqiyiProxyTestTask(Proxy proxy){
        this.proxy = proxy;
    }
    @Override
    public void run() {
        long startTime = System.currentTimeMillis();
        HttpGet request = new HttpGet(Constants.IQIYI_INDEX_URL);
        try {
            ...
            ProxyPool.iqiyiProxyQueue.add(proxy);
        } catch (IOException e) {
