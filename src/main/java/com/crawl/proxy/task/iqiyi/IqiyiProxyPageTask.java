...
import static com.crawl.proxy.ProxyPool.iqiyiProxyQueue;

/**
 * 下载代理网页并解析
 * 若下载失败，通过代理去下载代理网页
 */
public class IqiyiProxyPageTask implements Runnable {
    ...
    protected static IqiyiProxyHttpClient proxyHttpClient = IqiyiProxyHttpClient.getInstance();

    private IqiyiProxyPageTask() {

    }

    public IqiyiProxyPageTask(String url, boolean proxyFlag) {
        this.url = url;
    ...
    public void run() {
        ...
        try {
            ...
            if (proxyFlag) {
                ...
                currentProxy = iqiyiProxyQueue.take();
            ...
            if (currentProxy != null) {
                ...
                iqiyiProxyQueue.add(currentProxy);
            }
            if (tempRequest != null) {
     ...
     */
    public void retry() {
        proxyHttpClient.getProxyDownloadThreadExecutor().execute(new IqiyiProxyPageTask(url, Config.iqiyiIsProxy));
    }
    ...
    public void handle(Page page) {
        ...
        for (Proxy p : proxyList) {
            if (!ProxyHttpClient.getInstance().getDetailListPageThreadPool().isTerminated()) {
                ...
                boolean containFlag = ProxyPool.iqiyiProxySet.contains(p);
                ...
                if (!containFlag) {
                    ...
                    proxyHttpClient.getProxyTestThreadExecutor().execute(new IqiyiProxyTestTask(p));
                }
            }
        }
    }
