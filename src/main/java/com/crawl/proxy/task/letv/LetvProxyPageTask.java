...
import static com.crawl.proxy.ProxyPool.letvProxyQueue;

/**
 * 下载代理网页并解析
 * 若下载失败，通过代理去下载代理网页
 */
public class LetvProxyPageTask implements Runnable {
    ...
    protected static LetvProxyHttpClient proxyHttpClient = LetvProxyHttpClient.getInstance();

    private LetvProxyPageTask() {

    }

    public LetvProxyPageTask(String url, boolean proxyFlag) {
        this.url = url;
    ...
    public void run() {
        ...
        try {
            ...
            if (proxyFlag) {
                ...
                currentProxy = letvProxyQueue.take();
            ...
            if (currentProxy != null) {
                ...
                letvProxyQueue.add(currentProxy);
            }
            if (tempRequest != null) {
     ...
     */
    public void retry() {
        proxyHttpClient.getProxyDownloadThreadExecutor().execute(new LetvProxyPageTask(url, Config.letvIsProxy));
    }
    ...
    public void handle(Page page) {
        ...
        for (Proxy p : proxyList) {
            if (!CommonHttpClient.getInstance().getDetailListPageThreadPool().isTerminated()) {
                ...
                boolean containFlag = ProxyPool.letvProxySet.contains(p);
                ...
                if (!containFlag) {
                    ...
                    proxyHttpClient.getProxyTestThreadExecutor().execute(new LetvProxyTestTask(p));
                }
            }
        }
    }
