 ...
 */
public class LetvProxyTestTask implements Runnable{
    private final static Logger logger = LoggerFactory.getLogger(LetvProxyTestTask.class);
    private Proxy proxy;
    public LetvProxyTestTask(Proxy proxy){
        this.proxy = proxy;
    }
    @Override
    public void run() {
        long startTime = System.currentTimeMillis();
        HttpGet request = new HttpGet(Constants.LETV_INDEX_URL);
        try {
            ...
            ProxyPool.letvProxyQueue.add(proxy);
        } catch (IOException e) {
