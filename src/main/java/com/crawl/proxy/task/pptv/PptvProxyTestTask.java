...
public class PptvProxyTestTask implements Runnable{
    private final static Logger logger = LoggerFactory.getLogger(PptvProxyTestTask.class);
    private Proxy proxy;
    public PptvProxyTestTask(Proxy proxy){
        this.proxy = proxy;
    }
    @Override
    public void run() {
        long startTime = System.currentTimeMillis();
        HttpGet request = new HttpGet(Constants.PPTV_INDEX_URL);
        try {
            ...
            ProxyPool.pptvProxyQueue.add(proxy);
        } catch (IOException e) {
