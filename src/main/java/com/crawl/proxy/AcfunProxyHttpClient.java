 ...
 * a站代理客户端
 */
public class AcfunProxyHttpClient extends AbstractHttpClient {
    private static final Logger logger = LoggerFactory.getLogger(AcfunProxyHttpClient.class);
    private volatile static AcfunProxyHttpClient instance;
    public static Set<Page> downloadFailureProxyPageSet = new HashSet<>(ProxyPool.proxyMap.size());

    public static AcfunProxyHttpClient getInstance(){
        if (instance == null){
            synchronized (AcfunProxyHttpClient.class){
                if (instance == null){
                    instance = new AcfunProxyHttpClient();
                }
            }
        }
        return instance;
    ...
    private void initProxy(){
        ...
        try {
            proxyArray = (Proxy[]) HttpClientUtil.deserializeObject(Config.acfunProxyPath);
